...
def submit_user_input():
    ...
    if user_input:
        ...
        if "thread_id" not in st.session_state:
            # Initialize thread with first user message
            st.session_state.thread_id = startAssistantThread(user_input, st.session_state.vector_id)
        else:
            addMessageToThread(st.session_state.thread_id, user_input)
...
def main():
    ...
    .assistant-message {
        background: linear-gradient(135deg, #e0e8f9 0%, #c8d9f3 100%);
        color: #2c3e50;
        margin-right: auto;
    ...
    with col2:
        ...
        # Create the input field with on_change callback
        user_input = st.text_input("💬 Ask your question:", key="user_input", on_change=submit_user_input)
        
        # Create the submit button
        st.button("Submit", key="submit_button", on_click=submit_user_input)
